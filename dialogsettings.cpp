...
void DialogSettings::initializeAudio(const QAudioDeviceInfo &deviceInfo)
{
    ...
    qDebug() << "Decoding file...";

    if (decoder) delete decoder;
    decoder = new QAudioDecoder(this);
    decoder->setAudioFormat(format);
    decoder->setSourceFilename("/home/samson/Desktop/queen/succ.wav");
...
void DialogSettings::readBuffer() {
    qDebug() << "Reading buffer...";

    QAudioBuffer buffer = decoder->read();
    if (starting) {
        starting = false;
        arr = QByteArray::fromRawData(static_cast<const char *>(buffer.constData()), buffer.byteCount());
    } else {
        arr.append(static_cast<const char *>(buffer.constData()), buffer.byteCount());
    }
    //decoder->read();

    /*
    QAudioBuffer buf = decoder->read();
    const int length = buf.byteCount();
    const char *data = buf.constData<char>();

    audio->start()->write(data, length);
    */
    ...
    //Buffer buf.write(data, length);
    audio->start(buf);*/
}

void DialogSettings::finished() {
